...
from libs.utils import seed, print_args
            ...
            for s in range(data.nSubjects):
                if s != t:
                    ...
                    if Xs is None:
                        ...
                        Xs = np.concatenate((Xs, tempX), axis=0)
                    ys = np.concatenate((ys, tempY), axis=0)

        # Feature Extraction
        fs = Covariances().transform(Xs)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MDRM")
    ...
    parser.add_argument('--output', type=str, default='MDRM')
