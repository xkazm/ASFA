...
from libs.eegnet import EEGNetBase, feat_classifier, cal_acc
...
def main(data: MIDataLoader,
    ...
    for t in range(data.nSubjects):
        ...
        netF, netC = trainSource(fs, ys, criterion, args)

        # test
        netF.eval().cpu()
        netC.eval().cpu()
        test_acc, test_kappa = cal_acc(ft, yt, netF, netC)
...
def trainSource(x: torch.Tensor,
    ...
    netF = EEGNetBase(channels=x.shape[2], timepoints=x.shape[3], kernelSize=args.kernel_size,
                      numFilters=args.num_filters, bottleneck_dim=args.bottleneck_dim).to(args.device)
    ...
    for epoch in range(args.num_epochs_s):
        ...
        netF.eval().cpu()
        netC.eval().cpu()
        train_acc, _ = cal_acc(x, y, netF, netC)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default='EEGNet')
