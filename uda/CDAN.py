...
def train(xs: torch.Tensor,
    ...
    domain_discriminator = DomainDiscriminator(input_dim=args.randomized_dim, hidden_dim=args.hidden_dim)
    ...
    for k, v in netC.named_parameters():
        param_group += [{'params': v, 'lr': learning_rate}]
    for k, v in domain_discriminator.named_parameters():
    ...
    adv_loss = ConditionalDomainAdversarialLoss(domain_discriminator=domain_discriminator, grl=grl,
    ...
    for iter in range(iters):
        ...
        transfer_loss = adv_loss(fxs, outs, fxt, outt, iter)
        ...
        if (iter + 1) % interval == 0:
            ...
            netB.eval().cpu()
            netC.eval().cpu()
            train_acc, _ = cal_acc(xs, ys, netF, netB, netC)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CDAN/CDAN-E")
    ...
    parser.add_argument('--output', type=str, default='CDAN-E')
