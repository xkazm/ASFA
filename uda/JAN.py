...
from libs.jan import JointMultipleKernelMaximumMeanDiscrepancy
...
def train(xs: torch.Tensor,
    ...
    mkmmd_loss = JointMultipleKernelMaximumMeanDiscrepancy(
        kernels=(
    ...
    for iter in range(iters):
        ...
        transfer_loss = mkmmd_loss(
            (fxs, F.softmax(outs, dim=1)),
            (fxt, F.softmax(outt, dim=1))
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--non-linear', type=bool, default=True)
    parser.add_argument('--trade_off', type=float, default=1.)

    parser.add_argument('--output', type=str, default='JAN')
