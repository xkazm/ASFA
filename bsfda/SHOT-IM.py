...
def trainTarget(netF: nn.Module,
    ...
    for epoch in range(args.num_epochs_t):
        ...
        loss = InformationMaximization(gent=True)(out)
        ...
        if (epoch + 1) % 10 == 0:
            log = "Epoch {}/{}: total_loss: {:.4f}".format(
                epoch + 1, args.num_epochs_t, loss.item()
            )
            print(log)
            args.out_log_file.write(log + '\n')
            args.out_log_file.flush()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="black-SHOT-IM")
    ...
    parser.add_argument('--output', type=str, default='black-SHOT-IM')
