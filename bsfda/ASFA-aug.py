...
from libs.loss import LabelSmooth, TEntropyLoss, ConsistencyLoss
from libs.augment import loadAugment, random_dropmin, augment
...
def main(data: MIDataLoader,
    ...
    for t in range(data.nSubjects):
        ...
        newF, newB, newC = trainTarget(netF, netB, netC, ft, args, data, t)

        # test
        newF.eval().cpu()
        newB.eval().cpu()
        newC.eval().cpu()
        test_acc = cal_acc(ft, yt, newF, newB, newC)
...
def trainTarget(netF: nn.Module,
    ...
    for epoch_d in range(args.num_epochs_d):
        x_weak = augment(data, subject, args, args.p_weak, args.lamb_weak)
        x_strong = augment(data, subject, args, args.p_strong, args.lamb_strong)
        x_weak, x_strong = x_weak.to(args.device), x_strong.to(args.device)
        out = newC(newB(newF(x_strong)))
        softmax_out = nn.Softmax(dim=1)(out)
        out_t = netC(netB(netF(x_weak)))
        softmax_out_t = nn.Softmax(dim=1)(out_t / args.t_d).detach()
        loss = torch.mean(((softmax_out - softmax_out_t) ** 2).sum(dim=1))
    ...
    for epoch in range(args.num_epochs_t):
        fx = newB(newF(x_device))
        out = newC(fx)
        softmax_out = nn.Softmax(dim=1)(out)

        loss_mcc = TEntropyLoss(t=args.t_mcc)(out)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="black-ASFA-aug")
    ...
    parser.add_argument('--low', type=float, default=0.0, help="dropmin low probability")
    parser.add_argument('--high', type=float, default=1.0, help="dropmin high probability")

    parser.add_argument('--p_weak', type=float, default=0.1, help="weak augment weaken probability")
    parser.add_argument('--lamb_weak', type=float, default=0.5, help="weak augment lowest bound")
    parser.add_argument('--p_strong', type=float, default=0.2, help="strong augment weaken probability")
    parser.add_argument('-lamb_strong', type=float, default=0.5, help="strong augment lowest bound")

    parser.add_argument('--output', type=str, default='black-ASFA-aug')
