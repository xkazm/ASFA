...
from libs.loss import LabelSmooth, Entropy, SKLDivLoss, ClassifierDiscrepancyLoss
...
def trainTarget(netF: nn.Module,
    ...
    newC.load_state_dict(netC.state_dict())
    ...
    n_samples, n_feature = x_device.shape[0], x_device.shape[1]
    for epoch in range(args.num_epochs_t):
        newC.train()
        netF.eval()
        netB.eval()
        for _ in range(args.iter_cast):
            fx = netB(netF(x_device))
            out_old = netC(fx)
            out = newC(fx)

            softmax_out_old = nn.Softmax(dim=1)(out_old)
            softmax_out = nn.Softmax(dim=1)(out)

            entropy = Entropy(softmax_out_old)
            indx = entropy.topk(int(n_samples * 0.5), largest=True)[-1]
            one_mask = torch.ones(n_samples).to(args.device)
            one_mask[indx] = -1

            # print(softmax_out)
            # print(softmax_out_old)

            loss_cast = SKLDivLoss()(softmax_out, softmax_out_old).sum(dim=1)
            # print(loss_cast)
            loss_cast = torch.mean(loss_cast * one_mask)
            ...
            if (epoch + 1) % 10 == 0:
                log = "Casting: Epoch {}/{}: loss cast: {:.4f}".format(epoch + 1, args.num_epochs_t,
                                                                       loss_cast.item())
                print(log)
                args.out_log_file.write(log + '\n')
                args.out_log_file.flush()
        ...
        for _ in range(args.iter_bite):
            ...
            loss_bite = ClassifierDiscrepancyLoss(balance=True)(softmax_out, softmax_out_old)
            ...
            if (epoch + 1) % 10 == 0:
                log = "Biting: Epoch {}/{}: loss bite: {:.4f}".format(epoch + 1, args.num_epochs_t, loss_bite.item())
                print(log)
                args.out_log_file.write(log + '\n')
                args.out_log_file.flush()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="BAIT")
    ...
    parser.add_argument('--iter_cast', type=int, default=1)
    parser.add_argument('--iter_bite', type=int, default=1)
